...
# mysql -h 192.168.2.8 -uroot -pxxx CMS_SZLM
...
def commit_data_rewrite_api_gitlog():
    ...
    ssh.connect(hostname='192.168.2.8', port=22, username=username, password=password)
                    ...
                    for ref_name in refs.decode('utf-8').split('\n'):
                        if ref_name:
                            ...
                            stdin, stdout, stderr = ssh.exec_command(
                                r"cd %s && git log %s --pretty=format:'%%H|%%an|%%ae|%%cn|%%ce|%%ci|%%s' --numstat" % \
                            ...
                            for da in data:
                                if "|" in da:
                                    ...
                                    data_insert = (
                                            ...
                                            (data_line['commit_hash'], data_line['author_name'],
                                             data_line['author_email'], \
                                             data_line['committer_name'], data_line['committer_email'],
                                             data_line['committer_date'], \
                                             data_line['namespace'], data_line['project'], data_line['ref'],
                                             data_line['subject'], \
    ...
    quit_name_list = ['刘家兴', '赵华飞', 'liujx', 'zhangxd', 'xewk', 'suiyang', 'Cailiang', 'YANG,ZhiGang', 'YANG Zhigang',
                      'lansj', 'WangChen', '269333685@qq.com', 'secpersu', 'shuzilm', 'tianxc', 'anxz', 'lipeigang',
                      'chenling', 'songguang', 'zhaohf', 'raoping', 'jiazhongqiang', 'System Administrator', 'jerryyyq',
                      'root', 'YANG', 'Andy841', 'Ghost', 'bruce']
    ...
    cursor.execute(
        "Insert into api_gitlogquit select * from api_gitlog where committer_name in %s" % str(quit_name_tuple))
...
def commit_data_rewrite_api_commits():
 ...
 17 | lock_version  | bigint(20)   | YES  |     | NULL    |                | 
    ...
    conn = pymysql.Connect(host='192.168.2.8', port=3306, user='root', password='Aeic3672@983cie', database='du_op_app',
                           charset='utf8')
    cursor = conn.cursor()
    ...
    for i in s:
        ...
        insert = "insert into api_commits values('%s',%s,'%s','%s','%s','%s','%s','%s',%s,'%s',%s,'%s','%s',%s,'%s',%s,'%s',%s)" % \
                 (i[0], i[9], i[1], i[2], i[3], '', i[4], i[5], i[6], i[7], i[8], i[22], i[10], i[11], i[12], i[13],
                  i[14], i[15])
        # 以下4行 为i的位置和对应的值
